...
- Susceptibility distortion correction is a significant source of analytical flexibility
    - Multiple fieldmap sequences (spin echo, gradient echo, scanner-reconstructed $B_0$ maps)
    - Multiple correction implementations with different data structures
- Comparability among methods is poor, with little insight into sources of variation

- SDCFlows
    - Defines an intermediate representation of a field inhomogeneity map
    - Wraps or implements methods for estimating this map from different sequences
    - Provides a single method for applying estimated maps to EPI sequences

- SDCFlows is used in fMRIPrep and dMRIPrep, with plans to make a standalone tool
  for custom processing or easier interoperation with other pipelines
